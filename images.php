				...
				if ($image['img_vs_folder'] == 2) {
					...
					if (count($rows) > 0) {
						...
						foreach ($rows as $row) {
							switch ($album['gall_type']) {
								case 1: case 2: case 3:
									echo '<img file_b="'.$siteurl.$row['img_path'].'" title="'.$row['img_caption'].'" url="'.$row['img_url'].'"><![CDATA['.$row['img_description'].']]></img>'; 
								break;
						...
						echo "</folder>";
							...
							switch ($album['gall_type']) {
								case 1: case 2: case 3:
									$nofolder .= '<img file_b="'.$siteurl.$image['img_path'].'" title="'.$image['img_caption'].'" url="'.$image['img_url'].'"><![CDATA['.$image['img_description'].']]></img>'; 
								break;
								case 4: case 5: case 6: case 8: case 9:
