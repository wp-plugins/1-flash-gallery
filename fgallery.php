...
if (is_writable(FGALLERY_ABSPATH.'/imagecache')){
	define('PREVIEW_PATH',WP_PLUGIN_URL.'/'.basename(dirname(__FILE__)).'/image.php/cover.jpg?width=300&amp;image='.EXTRA_DIR);
} else {
	...
	<div class="wrap">
			...
			<h2><?php _e('Server Settings', 'fgallery')?></h2>
                        <?php echo '<p><span class="lbl">'.__('Server Name:', 'fgallery').'</span>'.$_SERVER['SERVER_NAME']."</p>";
                               ...
                               if (!is_dir(ABSPATH . 'wp-content/plugins/'.basename(dirname(__FILE__)) .'/imagecache') || !is_writeable(ABSPATH . 'wp-content/plugins/'.basename(dirname(__FILE__)) .'/imagecache')) {
                                   echo '<p class="fgallery_error">'.__("Folder", 'fagllery').' wp-content/plugins/'.basename(dirname(__FILE__)) .'/imagecache '.__("is not writable", 'fgallery').'</p>';
                               }
                               if (FGALLERY_PHP4_MODE) {
                                            echo '<p class="fgallery_error">'.__("Your server runs PHP 4. Resizing images and watermarks are not available for this software version.", 'fgallery').'</p>';
                               }
                         ?>
		</div>
		...
		<div id="fgallery_updates" class="fgallery_box">
			...
			<?php $updated = false;
				  ...
				  if (get_option('1_flash_gallery_kranjc') != '') {
...
function fgallery_watermark_page() {
	...
	<div class="wrap">
		...
		<div class="fgallery_box"><?php _e('Note: Watermark is placed only on images that were uploaded from "Local Computer", "ZIP" and "FTP" tabs. Other images will not have watermark due to copyright reasons. Images that were uploaded with watermark will stay with it untill are deleted. Watermark could not be removed without file reuploading.', 'fgallery')?></div>
...
function fgallery_default_album_settings() {
    $settings = array();
		...
		$settings['sc_navigation__mode'] = 'allTime';
...
function fgallery_add_button($buttons) {
     $fgallery_button = " <a href='" . esc_url( fgallery_get_add_album_url() ) . "' id='insert_gallery' class='thickbox' title='".__('Insert gallery into post', 'fgallery')."'>
