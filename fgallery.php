...
function fgallery_render_album_item_row($album) {
	...
	$output = '<tr class="fgallery_album" id="album_'.$album['gall_id'].'">';
		...
		$output .= '<td><a href="'.fgallery_get_edit_url($id).'">'.html_entity_decode($album['gall_name']).'</a></td>';
		...
		$output .= '<td>'.html_entity_decode($album['gall_description']).'</td>';
...
function fgallery_render_image_item_row($image) {
	...
	if ($image['img_vs_folder']){
		...
		if ($image['img_vs_folder']) {
			...
			$output .= '<td><a href="'.fgallery_get_folder_url($id).'">'.html_entity_decode($image['img_caption']).'</a></td>';
			...
			$output .= '<td><a href="'.fgallery_get_image_edit_url($id).'">'.html_entity_decode($image['img_caption']).'</a></td>';
...
function fgallery_insert_gallery($content){
       $regex = '/\[\s*fgallery\s+id\=(\d+)\s+w\=(\d*)\s+h\=(\d*)\s+bg\=([a-fA-F0-9]*)\st=(\d*).*?\]/';     
...
function fgallery_replace_snippet($matches) {
		...
		switch ($type) {
			case '0' : 
				$path = fgallery_search_flash_path($album);
				$gallery_snippet = '<div style="width:'.$width.'px;">
				<script type="text/javascript">
					var flashvars = {settings: "'.FGALLERY_PATH.'/config.php?gall_id='.$id.'", images : "'.FGALLERY_PATH.'/images.php?gall_id='.$id.'"};
					var params = {bgcolor: "#'.$bgcolor.'", allowFullScreen: "true", wmode: "transparent"};
					swfobject.embedSWF("'.$path.'", "flashcontent_'.$id.'", "'.$width.'", "'.$height.'", "10.0.0",false, flashvars, params);
				  </script>
