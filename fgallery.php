...
define('FGALLERY_PATH', WP_PLUGIN_URL.'/fgallery');
define('FGALLERY_DIR', ABSPATH . 'wp-content/uploads/fgallery');
define('COVER_PATH',WP_PLUGIN_URL.'/fgallery/image.php/cover.jpg?width=200&amp;image='.EXTRA_DIR);
define('THUMB_PATH',WP_PLUGIN_URL.'/fgallery/image.php/cover.jpg?width=600&amp;image='.EXTRA_DIR);
...
function fgallery_install() {
   ...
   add_option('toplevel_page_fgallery_per_page', '25', '', 'no');
	...
	$create_albums = "CREATE TABLE " . ALBUMS_TABLE . " (
					    ...
					    `gall_type` smallint(3) default '0',
...
function fgallery_init() {
	...
	wp_register_script('uislider', FGALLERY_PATH.'/js/ui.slider.js',array('jquery'));
...
function fgallery_add_pages() {
	$pref = add_menu_page(__('1 Flash Gallery','fgallery'), __('1 Flash Gallery', 'fgallery'), 2, 'fgallery', 'fgallery_admin_albums');
	if ( function_exists('add_submenu_page') ) {
		$main   = add_submenu_page('fgallery', __('Galleries List','fgallery'), __('Galleries List','fgallery'), 2, 'fgallery', 'fgallery_admin_albums');
		$add    = add_submenu_page('fgallery', __('Create gallery','fgallery'), __('Create Gallery','fgallery'), 2, 'fgallery_add', 'fgallery_add_gallery');
		$images = add_submenu_page('fgallery', __('Images List','fgallery'), __('Images List','fgallery'), 2, 'fgallery_images', 'fgallery_images_page');
		$upload = add_submenu_page('fgallery', __('Upload images','fgallery'), __('Upload Images','fgallery'), 2, 'fgallery_upload', 'fgallery_upload_page');
	}
...
?>
	...
	<h2><?php _e('Upload images from:', 'fgallery');?></h2>
		<div id="upload_tabs">
				...
				<script type="text/javascript">
					jQuery(document).ready(function() {
						jQuery("#uploadify").uploadify({
							'uploader'       : '<?php echo WP_PLUGIN_URL ?>/fgallery/js/uploadify/uploadify.swf',
							'checkScript'    : '<?php echo WP_PLUGIN_URL ?>/fgallery/js/uploadify/check.php',
							'script'         : '<?php echo WP_PLUGIN_URL ?>/fgallery/js/uploadify/uploadify.php',
							'cancelImg'      : '<?php echo WP_PLUGIN_URL ?>/fgallery/images/cancel.png',
							'queueID'        : 'fileQueue',
							'fileDesc'       : 'Images',
			...
			<div id="ftp_tab">
				<form method="post" action="<?php echo fgallery_upload_ftp_get_url();?>">
			...
			<div id="zip_tab">
				<form method="post" action="<?php echo fgallery_upload_zip_get_url();?>" enctype="multipart/form-data">
			...
			<div id="wp_gall_tab">
				...
				<form method="post" action="<?php echo fgallery_upload_media_get_url();?>">
				<label for="fgallery_url_folder"><?php _e('Images will be saved to:','fgallery')?></label>
					<select name="fgallery_url_folder" id="fgallery_url_folder">
						<option value="0"><?php _e('Root folder', 'fgallery')?></option>
						<?php 
...
function fgallery_preferences_page() {
	...
	<div class="wrap">
		...
		<div id="fgallery_info" class="fgallery_box">
			<?php _e('Text about plugin','fgallery')?>
		</div>
		<div id="fgallery_faq" class="fgallery_box">
			<?php _e('FAQ links, video tutorials, configuration','fgallery')?>
		</div>
		<div id="fgallery_settings" class="fgallery_box">
			<h2><?php _e('Server Settings', 'fgallery')?></h2>
                        <?php echo '<p><span class="lbl">'.__('Server Name:', 'fagllery').'</span>'.$_SERVER['SERVER_NAME']."</p>";
                               ...
                               if (!is_dir(ABSPATH . 'wp-content/plugins/fgallery/imagecache') || !is_writeable(ABSPATH . 'wp-content/plugins/fgallery/imagecache')) {
                                   echo '<p class="fgallery_error">'.__("Folder", 'fagllery').'wp-content/plugins/fgallery/imagecache'.__("is not writable", 'fagllery').'</p>';
                               }
                         ?>
		</div>
...
function fgallery_edit_album_page($id){
	...
	if ($id != 0):
	        echo '<script type="text/javascript">
                ...
                swfobject.embedSWF("'.FGALLERY_PATH.'/swf/'.fgallery_get_flash_type($album).'.swf", "flashcontent", "'.$album['gall_width'].'", "'.$album['gall_height'].'", "10.0.0",false, flashvars, params);
              </script> ';
...
function fgallery_default_album_settings() {
    $settings = array();
        $settings['sc_slideshow__autostart'] = 1;
        $settings['sc_slideshow__enable'] = 1;
        $settings['sc_slideshow__delay'] = '2';
        $settings['sc_slideshow__stopByClick'] = 1;
		...
		$settings['sc_preview__shadow'] = 1;
		$settings['sc_preview__border'] = 1;
		...
		$settings['sc_screen__mainPreloader'] = 1;
		$settings['sc_screen__navigationsButton'] = 1;
		$settings['sc_screen__previewPreloader'] = 0;
	
        $settings['sc_scroller__distanceFromBorder'] = '30';
        $settings['sc_scroller__visible'] = 1;
        $settings['sc_scroller__scrollBy'] = 'mouse';
        $settings['sc_scroller__speed'] = 10;
        ...
        $settings['sc_scroller__enable'] = 1;
        ...
        $settings['sc_scroller__rectangleIndent'] = '5';
        $settings['sc_scroller__rectangleDistance'] = '6';
        $settings['sc_scroller__rectangleColor'] = 'f0f0f0';
        $settings['sc_scroller__rectangleAlpha'] = '0.7';
		$settings['sc_scroller__rectangleBorderSize'] = '1';
        $settings['sc_scroller__rectangleBorderColor'] = 'ffffff';
		$settings['sc_scroller__rectangleWidth'] = '80';
		$settings['sc_scroller__rectangleHeight'] = '40';
		$settings['sc_scroller__useImagesInRectangle'] = 1;
        ...
        $settings['sc_image__imageAsLink'] = 1;
        ...
        $settings['sc_image__stageBlackout'] = 1;
...
function fgallery_get_album_images($gall_id, $folder = 0) {
	global $wpdb;
		$items = $wpdb->get_results("SELECT a.* FROM " . IMAGES_TABLE . " as a LEFT JOIN ". IMAGES_TO_ALBUMS_TABLE." as b ON (a.img_id = b.img_id) WHERE b.gall_id = " .$gall_id." AND b.gall_folder = ".$folder." ORDER BY a.img_vs_folder DESC, b.img_order ASC", 'ARRAY_A');
	return $items;
}
...
function sc_params_pane($album) {
  ...
  $params_xml = simplexml_load_file(FGALLERY_PATH . '/xml/params_'.$type.'.xml');
  $settings_xml = simplexml_load_file(FGALLERY_PATH.'/config.php?gall_id='.$gall_id);

  // Preparing jQuery UI tabs
  $tab_controls = "<ul>";
	$tab_controls .= '<li><a href="#sc-group-main">' . __('General','fgallery') . '</a></li>';
...
function sc_controls_checkbox($p) {
  ...
  if ($checked == '' && $p['default'] == 'true') $checked = 'checked = "checked"';
  return '<div class="form-item ' . $p['zebra'] . '" >'
...
$output .= '<tfoot>
	...
	if (!empty($albums)) {
		...
		$output .= '<tr><td colspan="7">'.__('There are no albums yet. You can create new one using Add gallery menu','fgallery'). '</td></tr>';
	}
	$output .= '</tbody></table>';
...
function fgallery_render_album_item_row($album) {
	...
	$output = '<tr class="fgallery_album" id="album_'.$album['gall_id'].'">';
		...
		$output .= '<td><a href="'.fgallery_get_edit_url($id).'">'.html_entity_decode($album['gall_name'], ENT_NOQUOTES, "UTF-8").'</a></td>';
		$output .= '<td><a href="'.fgallery_get_edit_url($id).'">'.fgallery_get_album_cover($album).'</a></td>';
		$output .= '<td>'.html_entity_decode($album['gall_description'], ENT_NOQUOTES, "UTF-8").'</td>';
...
function fgallery_render_image_item_row($image) {
	...
	if ($image['img_vs_folder']){
		...
		if ($image['img_vs_folder']) {
			$output .= '<td><a href="'.fgallery_get_folder_url($id).'">'.html_entity_decode($image['img_caption'], ENT_NOQUOTES, "UTF-8").'</a></td>';
			$output .= '<td><a href="'.fgallery_get_folder_url($id).'"><img src="'.FGALLERY_PATH.'/images/folder.png" alt="'.$image['img_caption'].'"/></a></td>';
			$output .= '<td>'.html_entity_decode($image['img_description'], ENT_NOQUOTES, "UTF-8").'</td>';
			$output .= '<td class="folder_attr">'.fgallery_get_folder_attributes($image).'</td>';
			$output .= '<td>'.fgallery_get_folder_actions($id).'</td>';
		} else {
			$output .= '<td><a href="'.fgallery_get_image_edit_url($id).'">'.html_entity_decode($image['img_caption'], ENT_NOQUOTES, "UTF-8").'</a></td>';
...
function fgallery_get_listof_gallery_albums($id) {
	...
	if (count($albums) > 0) {
		$output = '<select name="album_id" id="album_id_images">';
			$output .= '<option value="">'.__('Choose album', 'fgallery').'</option>';
		foreach ($albums as $album) {
...
function fgallery_get_album_cover($album) {
global $wpdb;
$gall_id = $album['gall_id'];
	...
	if ($cover['img_path']!=''){
		$gall_cover = '<img src="'.COVER_PATH.$cover['img_path'].'" alt="'.$cover['img_caption'].'" />';
	}
	return $gall_cover;
}
...
function fgallery_do_shortcode($id, $type = 0) {
    $album = fgallery_get_album($id);
    return sprintf('[fgallery id=%d w=%d h=%d bg=%s t=%d]',$id, $album['gall_width'], $album['gall_height'], $album['gall_bgcolor'], $type);
}

// renders the gaellery edit form
function fgallery_render_edit_album_form($album, $source) {
	$id = $album['gall_id'];
	$output .= '<div id="sc-group-main" class="sc-skin-params-tab-panel">';
...
add_filter('the_content', 'fgallery_insert_gallery', 9);
...
function fgallery_insert_gallery($content){
       $regex = '/\[\s*fgallery\s+id\=(\d+)\s+w\=(\d*)\s+h\=(\d*)\s+bg\=([a-f0-9]*)\st=(\d*).*?\]/';     
    return preg_replace_callback($regex,'fgallery_replace_snippet',$content);
}

function fgallery_replace_snippet($matches) {
        $id = !empty($matches[1])?$matches[1]:'0';
		if ($id > 0) {$album = fgallery_get_album($id);} else {die();}
        $width = !empty($matches[2])?$matches[2]:'450';
        $height = !empty($matches[3])?$matches[3]: '385';
        $bgcolor = !empty($matches[4])?$matches[4]:'ffffff';
        $type = !empty($matches[5])? $matches[5]:'0';
		switch ($type) {
			case '0' : 
				$gallery_snippet = '<div style="width:'.$width.'px;"><script type="text/javascript">
					var flashvars = {settings: "'.FGALLERY_PATH.'/config.php?gall_id='.$id.'", images : "'.FGALLERY_PATH.'/images.php?gall_id='.$id.'"};
					var params = {bgcolor: "#'.$bgcolor.'", allowFullScreen: "true", wmode: "transparent"};
					swfobject.embedSWF("'.FGALLERY_PATH.'/swf/'.fgallery_get_flash_type($album).'.swf", "flashcontent_'.$id.'", "'.$width.'", "'.$height.'", "10.0.0",false, flashvars, params);
				  </script>
			...
			break;
			case '1' : 
				$gallery_snippet = '<a href="'.fgallery_view_gallery_url($id, $width, $height, $bgcolor).'" class="thickbox" title="'.$album['gall_name'].'">'.__('View', 'fgallery').' '.$album['gall_name'].'</a><p>'.fgallery_get_album_attributes($album, false).'</p>';
			break;
			case '2' :
				$gallery_snippet = '<a href="'.fgallery_view_gallery_url($id, $width, $height, $bgcolor).'" class="thickbox" title="'.$album['gall_name'].'">'.fgallery_get_album_cover($album).'</a><p>'.$album['gall_name'].'</p><p>'.fgallery_get_album_attributes($album, false).'</p>';
			break;
			default: 
				$gallery_snippet = '<div style="width:'.$width.'px;"><script type="text/javascript">
					var flashvars = {settings: "'.FGALLERY_PATH.'/config.php?gall_id='.$id.'", images : "'.FGALLERY_PATH.'/images.php?gall_id='.$id.'"};
