				...
				<script type="text/javascript">
					jQuery(document).ready(function() {
						jQuery("#uploadify").uploadify({
							...
							'checkScript'    : '<?php echo WP_PLUGIN_URL.'/'.basename(dirname(__FILE__)) ?>/js/uploadify/check.php',
...
function fgallery_preferences_page() {
	...
	<div class="wrap">
			...
			<h2><?php _e('Server Settings', 'fgallery')?></h2>
                        <?php echo '<p><span class="lbl">'.__('Server Name:', 'fagllery').'</span>'.$_SERVER['SERVER_NAME']."</p>";
							   ...
							   if (ini_get('allow_url_fopen') != 1 ) {
							       echo '<p class="fgallery_error">'.__("Set allow_url_fopen to 'On' in php.ini", 'fagllery').'</p>';
							   }
                         ?>
		</div>
		...
		<div id="fgallery_updates" class="fgallery_box">
			...
			<p><?php _e('To run updates we must be sure that server permissions allow it. Click "Prepare for updates" to allow update. You will need ftp account details.')?></p>
			<p><?php _e('After running updates don\'t forget to "Finish updates". This is made for security purposes.')?></p>
			<a href="<?php echo admin_url('admin.php?page=fgallery&action=ftp&allow=1')?>" class="fgallery_action"><?php _e('Prepare for updates')?></a>
			<a href="<?php echo admin_url('admin.php?page=fgallery&action=ftp&allow=0')?>" class="fgallery_action"><?php _e('Finish updates')?></a>
			<h2><?php _e('Already Installed Updates', 'fgallery')?></h2>
			<?php if (get_option('1_flash_gallery_acosta') != '') {
...
if (isset($_GET['action'])) {
	...
	switch ($action) {
		case 'ftp':
			...
			$url = 'admin.php?page=fgallery&action=ftp&allow='.$allow;
			$url = wp_nonce_url($url, 'allow-ftp');
			
			if ( false === ($credentials = request_filesystem_credentials($url, 'ftp', false, ABSPATH)) )
			return;
			
			if ( ! WP_Filesystem($credentials, ABSPATH) ) {
				request_filesystem_credentials($url, 'ftp', true, ABSPATH); //Failed to connect, Error and request again
				return;
			}
			$plugins_dir = $wp_filesystem->wp_plugins_dir();
			$this_plugin_dir = trailingslashit( dirname($plugins_dir) .'/plugins/'.basename(dirname(__FILE__)) );
			$this_dir[] = $this_plugin_dir.'swf/acosta.swf'; 
			$this_dir[] = $this_plugin_dir.'swf/arai.swf'; 
			$this_dir[] = $this_plugin_dir.'swf/airion.swf'; 
			$this_dir[] = $this_plugin_dir.'swf/pax.swf'; 
			$this_dir[] = $this_plugin_dir.'swf/pazin.swf'; 
			$this_dir[] = $this_plugin_dir.'swf/postma.swf'; 
			$success = false;
			foreach ($this_dir as $file) {
				if ($allow) {
					if($wp_filesystem->chmod($file, 0777)) {
						//echo $file.' - '.__('Permissions are set to 777').'<br />';
						$success = true;
					} else {
						echo $file.' - '.__('Permission denied').'<br />';
					}
					
				} else {
					if ($wp_filesystem->chmod($file, 0644)) {
						echo $file.' - '.__('Permissions are set to 644').'<br />';
						//$success = true;
					} else {
						echo $file.' - '.__('Permission denied').'<br />';
					}
			...
			if ($success && $allow) {
				echo __(sprintf('Now go to <a href="%s">plugins</a> and activate updates',admin_url('plugins.php')));
			} else {
				echo __(sprintf('Now you can <a href="%s">go back</a> to galleries',admin_url('admin.php?page=fgallery')));
			}
		break;
		case 'pref':
...
function fgallery_replace_snippet($matches) {
		...
		switch ($type) {
			...
			break;
			case '1' : 
				preg_match('/title="(.*)"/',$matches[0],$text);
				if ($text[1] == '') {
					$insert_text = $album['gall_name'];
				} else {
					$insert_text = $text[1];
				}
				$gallery_snippet = '<a href="'.fgallery_view_gallery_url($id, $width, $height, $bgcolor).'" class="thickbox" title="'.$album['gall_name'].'">'.$insert_text.'</a><p>'.fgallery_get_album_attributes($album, false).'</p>';
			break;
			case '2' :
				preg_match('/thumb=(\d*)/',$matches[0],$thumb_width);
				$gallery_snippet = '<a href="'.fgallery_view_gallery_url($id, $width, $height, $bgcolor).'" class="thickbox" title="'.$album['gall_name'].'">'.fgallery_get_album_cover($album, $thumb_width[1]).'</a><p>'.$album['gall_name'].'</p><p>'.fgallery_get_album_attributes($album, false).'</p>';
			break;
			default: 
				$gallery_snippet = '<div style="width:'.$width.'px;"><script type="text/javascript">
					var flashvars = {settings: "'.FGALLERY_PATH.'/config.php?gall_id='.$id.'", images : "'.FGALLERY_PATH.'/images.php?gall_id='.$id.'"};
