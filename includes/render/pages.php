...
<div class="wrap fgallery">
    ...
    <div id="upload_tabs">
        ...
        <div id="first_tab">
                <label for="local_img_folder"><?php _e('Images will be saved to:','fgallery')?></label>
                <select id="local_img_folder">
                    <option value="0"><?php _e('Root folder', 'fgallery')?></option>
                    <?php if (!empty($folders)):
                              foreach ($folders as $item): 
                                    echo '<option value="'.$item['img_id'].'">'.$item['img_caption'].'</option>';
                              endforeach;  
                          endif;?>
                </select> <br />
...
function fgallery_settings_page() {
    if (isset($_POST) && wp_verify_nonce($_POST['fgallery_watermark_field'], 'fgallery_watermark')){
            ...
            update_option('1_flash_gallery_show_link',$_POST['link_opt']);
    ...
    <form id="fgallery_watermark_form" method="post" enctype="multipart/form-data">
        <fieldset>
            ...
            <fieldset>
                ...
                <label for="link_opt"><?php _e('Show link to http://1plugin.com', 'fgallery')?></label>
                <input type="checkbox" name="link_opt" id="link_opt" value="1" 
                    <?php if (get_option('1_flash_gallery_show_link',0) == 1) echo 'checked="checked"'?> /> 
            </fieldset>
    ...
    echo '<div class="wrap fgallery">';
        ...
        if ($id != 0):
            echo '<div class="edit_gallery_urls">';
                    $title = __('Add Images', 'fgallery');
                    echo '<a href="'.fgallery_get_addimages_page_url($id).'" 
...
function fgallery_edit_image_page($id){
    ...
    if ($image['img_vs_folder'] == 0) {
        ...
        echo '</div>
              <a href="'.get_option('siteurl').'/'.$image['img_path'].'" title="'.__('Full View','fgallery').'" target="_blank">
                        <img src="'.THUMB_PATH.$image['img_path'].'" alt="'.$image['img_caption'].'"/>
              </a><br />';
        if ($image['img_preview_path'] != '') {
              echo '<img src="'.get_option('siteurl').'/'.$image['img_preview_path'].'" alt="'.$image['img_caption'].'"/>';
        }
    }
    echo '</div>';
}
